...
''' [Level of Difficulty] = Input the level of difficulty of the sudoku puzzle.
Difficulty levels include ‘Easy’ ‘Medium’ ‘Hard’ and ‘Insane’. Outputs a sudoku
of desired difficulty.'''

class Cell():
    ''' Initilalizes Cell object. A Cell is a single box of a sudoku puzzle.
    81 Cells make up the body of a sudoku puzzle. Initializes puzzle with all
    possible answers available, solved to false, and position of Cell within
    the sudoku puzzle
    '''
    ...
    def checkPosition(self):
        ''' Returns the position of a Cell within a sudoku puzzle.
        x = row; y = col; z = box number.
        '''
    ...
    def setAnswer(self, num):
        ''' Sets an answer of a puzzle and sets a cell's solved method to True.
        This method also eliminates all other possible numbers.
        '''
...
def emptySudoku():
    ''' Creates an empty sudoku in row major form. Sets up all of the x, y,
    and z coordinates for the sudoku cells.
    '''
    ans = []
    for x in range(1,10):
        if x in [7,8,9]:
...
##Modified DF 2017-10-26
##def printSudoku(sudoku):
def strSudoku(sudoku):
    '''Returns a printable format of a sudoku grid. Format is on 9 character
    lines and digits on each line are grouped by 3 enclosed in square brackets.
    '''
...
##Following lines added
    for irow in range(1,10):
        line = "{0} {1} {2}".format(row1[0:3],row1[3:6],row1[6:10])
        strFormat = strFormat + line + "\n"
    print(strFormat)
    return strFormat
...
def sudokuChecker(sudoku):
    ''' Checks to see if an input a completed sudoku puzzle is of the correct
    format and abides by all of the rules of a sudoku puzzle.
    Returns True if the puzzle is correct, False if otherwise.
    '''
...
def perfectSudoku():
    '''Generates a completed sudoku. Sudoku is in the correct format and is
    completly random.
    '''
    result = False
    while result == False:
...
def solver(sudoku, f = 0):
    ''' Input an incomplete Sudoku puzzle and solver method will return the
    solution to the puzzle. First checks to see if any obvious answers can be
    set then checks the rows columns and boxes for obvious solutions. Lastly
    the solver 'guesses' a random possible answer from a random cell and checks
    to see if that is a possible answer. If the 'guessed' answer is incorrect,
    then it removes the guess and tries a different answer in a different cell
    and checks for a solution. It does this until all of the cells have been
    solved. Returns a printed solution to the puzzle and the number of guesses
    that it took to complete the puzzle. The number of guesses is a measure of
    the difficulty of the puzzle. The more guesses that it takes to solve a
    given puzzle the more challenging it is to solve the puzzle.'''
    if f > 900:
        return False
    guesses = 0
    ...
    while solvedCells != []:
        for n in solvedCells:
            ...
            ##now we itterate through the remaing unset cells and remove the
            ##input if it's in the same row, col, or box
            for i in cells:  
                ...
                if position1[2] == position2[2]:
                    copy_s[i].remove(finalValue)
                if copy_s[i].lenOfPossible() == 1 and i not in solvedCells \
                                                  and i in cells:
                    solvedCells.append(i)
                ##print(n)
...
def solve(sudoku, n = 0):
    ''' Uses the solver method to solve a puzzle. This method was built in order
    to avoid recursion depth errors.
...
####Modified below DF 2017-10-26
##def main(level):
def generate(level, printResults=False, levelTries=0):
    ...
    if level == 'Easy':
        ...
        s = puzzleGen(p)
        levelTries+=1
        print("level generated "+s[2])
        if s[2] != 'Easy':
            return generate(level, printResults, levelTries)
        t2 = time.time()
        t3 = t2 - t1
####modified
        prSudo = strSudoku(s[0])
        if printResults is True:
            print("Runtime is " + str(t3) + " seconds")
            print("Guesses: " + str(s[1]))
            print("Level: " + str(s[2]))
            print("{0} tries to generate a '{1}' sudoku".format(levelTries,level))
            print(prSudo)
        return prSudo
##        print("Runtime is " + str(t3) + " seconds")
##        print("Guesses: " + str(s[1]))
