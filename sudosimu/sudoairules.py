...
class SudoAIdata(dict, base.SudoBaseClass):
    ...
    '''
    def __init__(self, mem, env=None, testlevel=sudoenv.TEST_AIDATALEVEL):
        #init de la classe racine SudoBaseClass
        ...
        lev = env.testLevel("aidata")
        if lev is not None and testlevel != lev:
            testlevel = lev
        base.SudoBaseClass.__init__(self, env=env, \
                                    testlabel="aidata", testlevel=testlevel)
...
class SudoAIcritSet(base.SudoBaseClass):
    ...
    def __init__(self, mem, aidata, \
                 env=None, testlevel=sudoenv.TEST_AICRITLEVEL):
        ...
        assert isinstance(testlevel, int) and testlevel>=0 \
                or testlevel is None
        #reprendre un précédente niveau de test s'il existe déjà
        oldlev = env.testLevel("aicrit")
        if oldlev is not None and testlevel != oldlev:
            testlevel = oldlev
        base.SudoBaseClass.__init__(self, env=env, \
                                    testlabel="aicrit", testlevel=testlevel)
        #ok init de cette classe
        TEST = self.env.TEST
        TEST.display("aicrit", 3, "SudoAIcritSet - Dans __init__()")
        TEST.display("aicrit", 1, "SudoAIcritSet - Initialisation de l'ensemble "\
                                  "de critères du système de décision AI.")
        assert isinstance(mem, SudoMemory)
        self._mem = mem
        assert isinstance(aidata, SudoAIdata)
        self._aidata = aidata
        #Création du dictionnaire de critères
        TEST.display("aicrit", 3, "SudoAIcritSet - Création du dictionnaire "\
                              "de critères.")
    ...
    def add(self, crit):
        ...
        '''
        TEST = self.env.TEST
        TEST.display("aicrit", 3, "SudoAIcritSet - Dans add()")
    ...
    def clear(self):
        ...
        TEST = self.env.TEST
        TEST.display("aicrit", 3, "SudoAIcritSet - Dans clear()")
    ...
    def crit(self, critName):
        ...
        '''
        TEST = self.env.TEST
        TEST.display("aicrit", 4, "SudoAIcritSet - Dans crit() - Demande " \
                     "du critère \"%s\"" % (critName))
    ...
    def evalAll(self):
        ...
        TEST = self.env.TEST
        TEST.display("aicrit", 3, "SudoAIcritSet - Dans evalAll()")
        TEST.display("aicrit", 2, "Evaluation de l'ensemble des critères AI")
...
class SudoAIcrit(base.SudoBaseClass):
    ...
    def __init__(self, mem, name, funcEval, data, \
        ...
        #ok init de cette classe
        TEST = self.env.TEST
        TEST.display("aicrit", 3, "SudoAIcrit - Dans __init__()")
        assert isinstance(mem, SudoMemory)
        self._mem = mem
    ...
    def eval(self):
        ...
        '''
        TEST = self.env.TEST
        TEST.display("aicrit", 4, "SudoAIcrit - Dans eval() -" \
    ...
    def clear(self):
        ...
        TEST = self.env.TEST
        TEST.display("aicrit", 3, "SudoAIcrit - Dans clear()")
        self._val = None
...
def createCriteria(mem, data, env):
    ...
    crits = {
        ...
        AICRIT_GRIDCHECKED : \
            SudoAIcrit(mem, AICRIT_GRIDCHECKED, fEvalAIcritGridChecked, data, env=env), \
        AICRIT_INTECHCHRCGA : \
...
def fEvalAIcritGridChecked(data):
    '''Retourne 1 si la grille est vérifiée non terminée.'''
    return (1 if data.get(AIDATA_GRIDCHECKED) is True else 0)
...
class SudoAIruleSet(base.SudoBaseClass):
    ...
    def __init__(self, mem, aicrits, \
                 env=None, testlevel=sudoenv.TEST_AIRULELEVEL):
        '''Crée le dictionnaire de règles.'''
        #init de la classe racine SudoBaseClass
        assert isinstance(env, sudoenv.SudoEnv) or env is None
        assert isinstance(testlevel, int) and testlevel>=0 \
                or testlevel is None
        #reprendre un précédente niveau de test s'il existe déjà
        oldlev = env.testLevel("airule")
        if oldlev is not None and testlevel != oldlev:
            testlevel = oldlev
        base.SudoBaseClass.__init__(self, env=env, \
                                    testlabel="airule", testlevel=testlevel)
        #ok init de cette classe
        TEST = self.env.TEST
        TEST.display("airule", 3, "SudoAIruleSet - Dans __init__()")
        TEST.display("airule", 2, "SudoAIcritSet - Initialisation de l'ensemble "\
                     "de règles du système de décision AI.")
        assert isinstance(mem, SudoMemory)
        self._mem = mem
        assert isinstance(aicrits, SudoAIcritSet)
        self._aicrits = aicrits
        #Création du dictionnaire de règles
        TEST.display("airule", 3, "SudoAIruleSet - Création du dictionnaire de règles.")
    ...
    def add(self, rule):
        ...
        '''
        TEST = self.env.TEST
        TEST.display("airule", 3, "SudoAIruleSet - Dans add()")
    ...
    def clear(self):
        ...
        TEST = self.env.TEST
        TEST.display("airule", 3, "SudoAIruleSet - Dans clear()")
    ...
    def rule(self, ruleName):
        ...
        '''
        TEST = self.env.TEST
        TEST.display("airule", 3, "SudoAIruleSet - Dans rule()")
    ...
    def ruleEval(self, ruleName):
        ...
        '''
        TEST = self.env.TEST
        TEST.display("airule", 3, "SudoAIruleSet - Dans ruleEval()")
        r = self._rules.get(ruleName, False)
    ...
    def evalAll(self):
        ...
        TEST = self.env.TEST
        TEST.display("airule", 3, "SudoAIruleSet - Dans evalAll()")
    ...
    def show(self):
        ...
        TEST = self.env.TEST
        TEST.display("airule", 3, "SudoAIruleSet - Dans show()")
...
class SudoAIrule(base.SudoBaseClass):
    ...
    '''
    def __init__(self, mem, name, text, funcEval, aicrits, \
                 env=None, testlevel=sudoenv.TEST_AIRULELEVEL):
        ...
        #ok init de cette classe
        TEST = self.env.TEST
        TEST.display("airule", 3, "SudoAIrule - Dans __init__()")
        assert isinstance(mem, SudoMemory)
        self._mem = mem
        assert isinstance(aicrits, SudoAIcritSet)
        self._aicrits = aicrits
    ...
    def eval(self):
        ...
        '''
        TEST = self.env.TEST
        TEST.display("airule", 4, "SudoAIrule - Dans eval()")
        try:
    ...
    def clear(self):
        ...
        TEST = self.env.TEST
        TEST.display("airule", 3, "SudoAIrule - Dans clear()")
        self._val = None
...
def createRules(mem, crits, env):
    ...
    rules = {
        AIRULE_CHECKGRID : \
            SudoAIrule(mem, AIRULE_CHECKGRID, AIRULETEXT_CHECKGRID, \
                       fEvalAIruleCheckGrid, crits, env=env), \
        AIRULE_TECHCONT : \
            SudoAIrule(mem, AIRULE_TECHCONT, AIRULETEXT_TECHCONT, \
                       fEvalAIruleTechCont, crits, env=env), \
        AIRULE_OPPORT : \
            SudoAIrule(mem, AIRULE_OPPORT, AIRULETEXT_OPPORT, \
                       fEvalAIruleOpport, crits, env=env), \
        AIRULE_CHRCGA : \
            SudoAIrule(mem, AIRULE_CHRCGA, AIRULETEXT_CHRCGA, \
                       fEvalAIruleChrcga, crits, env=env), \
        AIRULE_LPLCG : \
            SudoAIrule(mem, AIRULE_LPLCG, AIRULETEXT_LPLCG, \
                       fEvalAIruleLplcg, crits, env=env), \
        AIRULE_LPLCP : \
            SudoAIrule(mem, AIRULE_LPLCP, AIRULETEXT_LPLCP, \
                       fEvalAIruleLplcp, crits, env=env), \
        AIRULE_DISCARD : \
            SudoAIrule(mem, AIRULE_DISCARD, AIRULETEXT_DISCARD, \
                       fEvalAIruleDiscard, crits, env=env), \
        AIRULE_DISCARDALL : \
            SudoAIrule(mem, AIRULE_DISCARDALL, AIRULETEXT_DISCARDALL, \
                       fEvalAIruleDiscardAll, crits, env=env), \
        AIRULE_TECHABORT : \
            SudoAIrule(mem, AIRULE_TECHABORT, AIRULETEXT_TECHABORT, \
                       fEvalAIruleTechAbort, crits, env=env) \
        }
    return rules
...
def fEvalAIruleTechCont_Test():
    ...
    TEST.display("airule", 3, "SudoAI - dans fEvalAIruleTechCont.")
    val = ( \
...
def fEvalAIruleCheckGrid(crits, env):
    '''Règle de vérification de grille complète. La règle est toujours valide
    quand l'état de la grille n'est pas vérifié, indépendamment de tout
    autre critère.
    '''
    TEST = env.TEST
    TEST.display("airule", 3, "SudoAI - dans fEvalAIruleCheckGrid().")
    assert isinstance(crits, SudoAIcritSet)
    #critères contributifs
    if crits.crit(AICRIT_GRIDCHECKED).val == 0:
        val = 1.0
    else:
        val = 0.0
    TEST.display("airule", 2, "AI - Règle AIRULE_CHECKGRID : valeur = %i" % (val))
    return val
...
def fEvalAIruleTechCont(crits, env):
    ...
    TEST.display("airule", 2, "AI - Règle AIRULE_TECHCONT : valeur = %i" % (val))
    return val

def fEvalAIruleOpport(crits, env):
    '''Règle de possibilité de technique d'opportunité. Il est possible de
    lancer une technique de recherche d'opportunité après un placement et
    si le niveau d'imbrication est inférieur au max.
    C'est l'instance SudoAI qui gère la case sur laquelle la technique
    d'opportunité doit être appliquée (pas de paramètre pour cela dans
    ce module).
    '''
    TEST = env.TEST
    TEST.display("airule", 3, "SudoAI - dans fEvalAIruleOpport.")
    assert isinstance(crits, SudoAIcritSet)
    #les critères impératifs (dont l'évaluation DOIT être 1)
    if crits.crit(AICRIT_LASTACTPLACE).val == 0:
        return 0.0
    #les critères exclusifs (dont l'évaluation DOIT être 0)
    if crits.crit(AICRIT_NIVMAX).val > 0 \
       or crits.crit(AICRIT_NOTECH).val > 0:
        return 0.0
    #évaluation
    val = 1.0 * crits.crit(AICRIT_LASTACTPLACE).val
    val = min(val, 1.0) #saturation
    TEST.display("airule", 2, "AI - Règle AIRULE_OPPORT : valeur = %i" % (val))
    return val

def fEvalAIruleChrcga(crits, env):
    '''Règle de commencement de la technique globale ChRCga.
    C'est exclusivement une règle de premier niveau car elle est globale sur la
    grille donc n'est pas adaptée à une situation d'opportunité.
    Note : C'est la tactique AI qui choisit quelle règle de premier niveau
    appliquer quand plusieurs sont valides.
    '''
    TEST = env.TEST
    TEST.display("airule", 3, "SudoAI - dans fEvalAIruleChrcga()")
    assert isinstance(crits, SudoAIcritSet)
    ...
    if crits.crit(AICRIT_INTECH).val > 0:
        return 0
    #hormis ces exclusions la technique peut toujours être appliquée
    val = 1.0
    TEST.display("airule", 2, "AI - Règle AIRULE_CHRCGA : valeur = %i" % (val))
    return val
...
def fEvalAIruleLplcg(crits, env):
    ...
    '''
    TEST = env.TEST
    TEST.display("airule", 3, "SudoAI - dans fEvalAIruleLplcg()")
    assert isinstance(crits, SudoAIcritSet)
    #critères impératifs (dont l'évaluation DOIT être 1)
    if crits.crit(AICRIT_NIV0).val == 0 \
       or crits.crit(AICRIT_NOTECH).val == 0 :
    ...
    #hormis ces exclusions la technique peut toujours être appliquée
    val = 1.0
    TEST.display("airule", 2, "AI - Règle AIRULE_LPLCG: valeur = %i" % (val))
    return val

def fEvalAIruleLplcp(crits, env):
    '''Règle de commencement de la technique locale Lplcp. Celle-ci peut être
    lancée après un placement et à condition que le niveau actuel d'imbrication
    soit inférieur au max.
    '''
    TEST = env.TEST
    TEST.display("airule", 3, "SudoAI - dans fEvalAIruleLplcp()")
    assert isinstance(crits, SudoAIcritSet)
    
    #les critères impératifs (dont l'évaluation DOIT être 1)
    #La dernière action doit être un placement
    if crits.crit(AICRIT_LASTACTPLACE).val == 0:
        return 0.0
    #les critères exclusifs (dont l'évaluation DOIT être 0)
    #le niveau actuel ne doit pas être le max.
    if crits.crit(AICRIT_NIVMAX).val > 0 \
       or crits.crit(AICRIT_NOTECH).val > 0:
        return 0.0
    #évaluation
...
def fEvalAIruleDiscard(crits, env):
    ...
    '''
    TEST = env.TEST
    TEST.display("airule", 3, "SudoAI - dans fEvalAIruleDiscard().")
    assert isinstance(crits, SudoAIcritSet)
    #critères impératifs (dont l'évaluation DOIT être 1)
    #Il doit y avoir une technique en cours
    if crits.crit(AICRIT_INTECH).val == 0 :
        return 0
    #critères exclusifs (dont l'évaluation doit être 0)
    #Le niveau ne doit pas être 0
    if crits.crit(AICRIT_NIV0).val > 0:
        return 0
    #critères contributifs
    val = 1.0 * crits.crit(AICRIT_LASTACTEND).val
    val = min(val, 1.0) #saturation
    TEST.display("airule", 2, "AI - Règle AIRULE_DISCARD : valeur = %i" % (val))
    return val
...
def fEvalAIruleDiscardAll(crits, env):
    ...
    '''
    TEST = env.TEST
    TEST.display("airule", 3, "SudoAI - dans fEvalAIruleDiscardAll().")
    assert isinstance(crits, SudoAIcritSet)

#### ACTUELLEMENT ON N'UTILISE PAS CETTE REGLE
    val = 0
    TEST.display("airule", 2, "AI - Règle AIRULE_DISCARDALL : valeur = %i" % (val))
    return val
######
    #critères impératifs (dont l'évaluation DOIT être 1)
...
def fEvalAIruleTechAbort(crits, env):
    ...
    '''
    TEST = env.TEST
    TEST.display("airule", 3, "SudoAI - dans fEvalAIruleTechAbort.")
    assert isinstance(crits, SudoAIcritSet)
#### ACTUELLEMENT REGLE INUTILISEE ##
    TEST.display("airule", 2, "AI - Règle AIRULE_TECHABORT : valeur = %i" % (0))
    return 0
######
##    crits = ai.crits
##    #les critères impératifs (dont l'évaluation DOIT être 1)
